...
<section id="comment_id_{$oComment->getId()}" class="comment
	...
	<div class="info">
		...
		{/if}
		<br />
		<div class="date"><time datetime="{date_format date=$oComment->getDate() format='c'}">{date_format date=$oComment->getDate() hours_back="12" minutes_back="60" now="60" day="day H:i" format="j F Y, H:i"}</time></div>
