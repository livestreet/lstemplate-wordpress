{foreach from=$aWall item=oWall}
	...
	<div id="wall-item-{$oWall->getId()}" class="js-wall-item comment-wrapper">
    	...
    	<a href="{$oWallUser->getUserWebPath()}"><img src="{$oWallUser->getProfileAvatarPath(48)}" alt="avatar" width="40px" class="avatar" /></a>

    	<div class="info">
    		<span class="username"><a href="{$oWallUser->getUserWebPath()}">{$oWallUser->getLogin()}</a></span>
            {if $oWall->isAllowDelete()}
			    <a href="#" onclick="return ls.wall.remove({$oWall->getId()});" class="delete">{$aLang.wall_action_delete}</a>
			{/if}
    		<br />
    		<div class="date"><time datetime="{date_format date=$oWall->getDateAdd() format='c'}">{date_format date=$oWall->getDateAdd() hours_back="12" minutes_back="60" now="60" day="day H:i" format="j F Y, H:i"}</time></div>
    	</div>
